...
class RotaryPositionalEmbedding(nn.Module):
    def __init__(self, theta: float, d_k: int, max_seq_len: int, device=None):
        ...
        frequency = 1.0 / torch.pow(torch.tensor(theta), torch.arange(0, d_k, 2).float() / d_k)
        positions = torch.arange(max_seq_len)
        angles = torch.outer(positions, frequency)
        self.register_buffer("cos_cache", torch.cos(angles).to(device))
        self.register_buffer("sin_cache", torch.sin(angles).to(device))
